...
{
    //타일 오브젝트 생성하는 함수
    private void tileSpawner(string tileName, int user,string tagName, Vector3 position, Vector3 scale, GameObject ownGameObject)
    {
    ...
    public void handDisplay(List<string> _hand, int _user)
    {
        ...
        {
            tileSpawner(tile, _user,_user.ToString(), new Vector3(xPos, -4f, 0), new Vector3(0.75f, 0.75f, 0), userHand);
            xPos += 0.5f;
        }
        //유저 위치에 맞게 방향 돌리기
    ...
    public void tsumoDisplay(string _tile, int _user)
    {
        ...
        GameObject userHand = GameObject.Find(handName);
        userHand.transform.rotation = Quaternion.Euler(new Vector3(0, 0, 0));
        tileSpawner(_tile, _user, _user.ToString(), new Vector3(3.8f, -4.0f, 0), new Vector3(0.75f, 0.75f, 0), userHand);
        //유저 위치에 맞게 방향 돌리기
        userHand.transform.rotation = Quaternion.Euler(new Vector3(0, 0, _user * 90));
    }
    public void dahaiDisplay(string _tile,int _user, int howMany)
    {
        string userName = "User" + _user + "Discard";
        ...
        tileSpawner(_tile, 0, "Discard", new Vector3(yPos[y], xPos[x], 0), new Vector3(0.5f, 0.5f, 0), userDiscard);
        //유저 위치에 맞게 방향 돌리기
    ...
    public void doraDisplay(string _tile, int howMany, bool isOpen)
    {
        ...
        tileSpawner(_tile, isBack, "Dora", new Vector3(yPos[howMany], 4.55f, 0), new Vector3(1f, 1f, 0), dora);
    }
}
//ㄱ 장르 ab커플링